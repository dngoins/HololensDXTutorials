...
hlDX::DeviceResourcesWindowed::DeviceResourcesWindowed() :
    m_d3dViewport(),
...
void hlDX::DeviceResourcesWindowed::CreateDeviceResources()
{
...
void hlDX::DeviceResourcesWindowed::CreateWindowSizeDependentResources()
{
...
void hlDX::DeviceResourcesWindowed::UpdateRenderTargetSize()
{
   m_effectiveDpi = m_dpi;

   // To improve battery life on high resolution devices, render to a smaller render target
   // and allow the GPU to scale the output when it is presented.
   if (!hlDisplayMetrics::SupportHighResolutions && m_dpi > hlDisplayMetrics::DpiThreshold)
   {
      float width = hlDX::ConvertDipsToPixels(m_logicalSize.Width, m_dpi);
      float height = hlDX::ConvertDipsToPixels(m_logicalSize.Height, m_dpi);

      if (width > hlDisplayMetrics::WidthThreshold && height > hlDisplayMetrics::HeightThreshold)
      {
   ...
   m_outputSize.Width = hlDX::ConvertDipsToPixels(m_logicalSize.Width, m_effectiveDpi);
   m_outputSize.Height = hlDX::ConvertDipsToPixels(m_logicalSize.Height, m_effectiveDpi);
...
void hlDX::DeviceResourcesWindowed::SetWindow(CoreWindow^ window)
{
...
void hlDX::DeviceResourcesWindowed::SetSwapChainPanel(SwapChainPanel^ panel)
{
    DisplayInformation^ currentDisplayInformation = DisplayInformation::GetForCurrentView();
...
void hlDX::DeviceResourcesWindowed::SetLogicalSize(Windows::Foundation::Size logicalSize)
{
...
void hlDX::DeviceResourcesWindowed::SetDpi(float dpi)
{
   if (dpi != m_dpi)
   {
      m_dpi = dpi;
...
void hlDX::DeviceResourcesWindowed::SetCurrentOrientation(DisplayOrientations currentOrientation)
{
...
void hlDX::DeviceResourcesWindowed::SetCompositionScale(float compositionScaleX, float compositionScaleY)
{
...
void hlDX::DeviceResourcesWindowed::ValidateDevice()
{
   ...
   hlDX::ThrowIfFailed(dxgiDevice->GetAdapter(&deviceAdapter));

   ComPtr<IDXGIFactory4> deviceFactory;
   hlDX::ThrowIfFailed(deviceAdapter->GetParent(IID_PPV_ARGS(&deviceFactory)));

   ComPtr<IDXGIAdapter1> previousDefaultAdapter;
   hlDX::ThrowIfFailed(deviceFactory->EnumAdapters1(0, &previousDefaultAdapter));

   DXGI_ADAPTER_DESC1 previousDesc;
   hlDX::ThrowIfFailed(previousDefaultAdapter->GetDesc1(&previousDesc));

   // Next, get the information for the current default adapter.

   ComPtr<IDXGIFactory4> currentFactory;
   hlDX::ThrowIfFailed(CreateDXGIFactory1(IID_PPV_ARGS(&currentFactory)));

   ComPtr<IDXGIAdapter1> currentDefaultAdapter;
   hlDX::ThrowIfFailed(currentFactory->EnumAdapters1(0, &currentDefaultAdapter));

   DXGI_ADAPTER_DESC1 currentDesc;
   hlDX::ThrowIfFailed(currentDefaultAdapter->GetDesc1(&currentDesc));
...
void hlDX::DeviceResourcesWindowed::HandleDeviceLost()
{
   m_swapChain = nullptr;
...
void hlDX::DeviceResourcesWindowed::Present()
{
...
void hlDX::DeviceResourcesWindowed::SetD3DDevice(ID3D11Device3 * pDevice, ID3D11DeviceContext3* pDeviceContext)
{
    if (m_deviceNotify != nullptr)
    {
        m_deviceNotify->OnDeviceLost();
...
DXGI_MODE_ROTATION hlDX::DeviceResourcesWindowed::ComputeDisplayRotation()
{
...
void hlDX::DeviceResourcesWindowed::CreateViewProjection()
{
...
void hlDX::DeviceResourcesWindowed::UpdateViewProjectionBuffer()
{
...
bool hlDX::DeviceResourcesWindowed::AttachViewProjectionBuffer()
{
    // Loading is asynchronous. Resources must be created before they can be updated.
