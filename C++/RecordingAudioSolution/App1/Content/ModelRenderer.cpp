...
#include "Common\hlDirectXHelper.h"
...
ModelRenderer::ModelRenderer(const std::shared_ptr<hlDX::DeviceResources>& deviceResources, const std::wstring& filename, bool useRelativePath) :
...
void ModelRenderer::PositionHologram(float3 headPosition, float3 headDirection)
{
	// The hologram is positioned two meters along the user's gaze direction.
	 float distanceFromUserX = 1.0f; // meters
	 float distanceFromUserY = 1.0f;
	 float distanceFromUserZ = 2.0;
	
//	 float3 distance = float3(distanceFromUserX * headDirection.x, dis)
	 headDirection.z *= 2.0f;
	 const float3 gazeAtTwoMeters = float3(headPosition.x + headDirection.x, headPosition.y + headDirection.y, headPosition.z + headDirection.z);
...
void ModelRenderer::Update(const hlDX::StepTimer& timer)
{
...
void ModelRenderer::Render(bool isStereo)
{
	...
	const UINT stride = sizeof(MatMeshModLibrary::VertexPositionColor);
	const UINT offset = 0;
...
void ModelRenderer::CreateDeviceDependentResources()
{
	...
	task<std::vector<byte>> loadVSTask = hlDX::ReadDataAsync(vertexShaderFileName);
	task<std::vector<byte>> loadPSTask = hlDX::ReadDataAsync(L"ModelPixelShader.cso");

	task<std::vector<byte>> loadGSTask;
	if (!m_usingVprtShaders)
	{
		// Load the pass-through geometry shader.
		loadGSTask = hlDX::ReadDataAsync(L"ModelGeometryShader.cso");
	}
...
*/
	...
	{
		loadModelTask = hlDX::ReadDataAsync(m_filename);
	}
	else
	{
		loadModelTask = hlDX::ReadFullPathDataAsync(m_filename);
	}
...
void ModelRenderer::CreateVertexBuffer(ID3D11Device* device, const MatMeshModLibrary::Mesh& mesh, ID3D11Buffer** vertexBuffer) const
{
	...
	std::vector<MatMeshModLibrary::VertexPositionColor> vertices;
	...
	if (mesh.VertexColors().size() > 0)
	{
		...
		for (UINT i = 0; i < sourceVertices.size(); i++)
		{
			...
			vertices.push_back(MatMeshModLibrary::VertexPositionColor(XMFLOAT4(position.x, position.y, position.z, 1.0f),  color4));
		}
	}
	else
	{
	...
	vertexBufferDesc.ByteWidth = sizeof(MatMeshModLibrary::VertexPositionColor) * vertices.size();
