...
struct VertexShaderOutput
{
    ...
    uint        viewId      : TEXCOORD1;  // SV_InstanceID % 
	min16float2	textCoord	: TEXCOORD0;
...
float4x4 RotationMatrixY(float rotation)
{
...
float4x4 RotationMatrixX( float rotation)
{
	float c = cos(rotation);
	float s = sin(rotation);
...
float4x4 RotationMatrixZ(float rotation)
{
	float c = cos(rotation);
	float s = sin(rotation);
...
VertexShaderOutput main(VertexShaderInput input)
{
	...
	float4x4 vpMat = viewProjection[idx];
	//vpMat =mul(vpMat, RotationMatrixY(-90.f));
	//vpMat = mul(vpMat, RotationMatrixX(90.f));
	//vpMat = mul(vpMat, RotationMatrixZ(-0.5f));
