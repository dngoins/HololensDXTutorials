    ...
    SpatialCoordinateSystem^ coordinateSystem    )
{
    ...
    if (viewTransformAcquired)
    {
		...
		const float radiansY = static_cast<float>(fmod(90, XM_2PI));
		const float radiansX = static_cast<float>(fmod(45, XM_2PI));
	//	const float radiansZ = static_cast<float>(fmod(0.50, XM_2PI));

		XMMATRIX modelRotation = XMMatrixRotationY(-radiansY);
		modelRotation = modelRotation * XMMatrixRotationX(radiansX); // *XMMatrixRotationX(-radiansZ);;
        ...
        // every frame.
			...
			//	XMMatrixTranspose(XMMatrixMultiply(XMLoadFloat4x4(&viewCoordinateSystemTransform.Left) * XMLoadFloat4x4(&cameraProjectionTransform.Left), modelRotation))
			//	//XMLoadFloat4x4(&viewCoordinateSystemTransform.Left) * XMLoadFloat4x4(&cameraProjectionTransform.Left)
			//);
			//XMStoreFloat4x4(
			//	&viewProjectionConstantBufferData.viewProjection[1],
			//	XMMatrixTranspose(XMMatrixMultiply(XMLoadFloat4x4(&viewCoordinateSystemTransform.Right) * XMLoadFloat4x4(&cameraProjectionTransform.Right), modelRotation))
