...
void RemotingPage::Select_Model(Platform::Object ^ sender, Windows::UI::Xaml::RoutedEventArgs ^ e)
{
	...
	picker->FileTypeFilter->Append(".ply");
	picker->FileTypeFilter->Append(L".fbx");
	picker->FileTypeFilter->Append(L".obj");
	picker->FileTypeFilter->Append(L".lwo");
	//picker->FileTypeFilter->Append(L".stl");
	picker->FileTypeFilter->Append(L".3ds");
	picker->FileTypeFilter->Append(L".blend");
	picker->FileTypeFilter->Append(L".dae");
	picker->FileTypeFilter->Append(L".x3d");

	task<Windows::Storage::StorageFile^> loadModelTask = create_task( picker->PickSingleFileAsync());
	loadModelTask.then([=](Windows::Storage::StorageFile ^ file)
	{	
		tbModelFileName->Text =  file->Path;
		m_appView->SetModelFileName(file->Path, false);
		m_fileNameSet = true;
	});
...
void RemotingPage::Start_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	...
	InitAppView();
	if (!tbModelFileName->Text->IsEmpty())
	{
		m_fileNameSet = true;
		m_appView->SetModelFileName(tbModelFileName->Text, false);
	}
	...
	else if (!m_fileNameSet)
	{
		ConsoleLog(Console, L"Error: Please select a model");
	}
    else
    {
