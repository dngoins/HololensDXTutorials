...
void AppMain::SetHolographicSpace(HolographicSpace^ holographicSpace)
{
	...
	m_modelRenderer = std::make_unique<ModelRenderer>(m_deviceResources, m_modelFileName->Data(), m_useRelativePath);
...
HolographicFrame^ AppMain::Update()
{
    ...
    if (tapped)
    {
        ...
        m_spinningCubeRenderer->PositionHologram(            tapped->TryGetPointerPose(currentCoordinateSystem)            );

		m_modelRenderer->PositionHologram(tapped->TryGetPointerPose(currentCoordinateSystem));
    }
    else
    {
    ...
    for (auto cameraPose : prediction->CameraPoses)
    {
		...
		if(m_modelRenderer->IsLoaded())
		{
			m_modelRenderer->Pause();
			m_spinningCubeRenderer->Pause();
			DirectX::XMFLOAT4 color = { 0, 0, 0, 1 };
			m_spinningCubeRenderer->SetColorFilter(color);
			renderingParameters->SetFocusPoint(
				currentCoordinateSystem,
				m_modelRenderer->GetPosition()
			);
...
bool AppMain::Render(Windows::Graphics::Holographic::HolographicFrame^ holographicFrame)
{
        ...
        [this, holographicFrame](std::map<UINT32, std::unique_ptr<DX::CameraResources>>& cameraResourceMap)
    {
		...
		//prediction->CameraPoses->GetAt(0)->ProjectionTransform.
